...
class Shapes3dClassDataset(data.Dataset):
    def __init__(self, dataset_folder, fields, binIdx, split=None,
        ...
        if split is None:
            ...
            with open(split_file, 'r') as f:
                models = f.read().split('\n')
                models = models[:-1] # LINE ADDED BY ERIK SANDSTROEM do I need this line anymore?
    ...
    def __getitem__(self, idx): # this function takes as input an index and then in the models, an object ID is retrieved.
        model = self.models[idx]
        model_path = os.path.join(self.dataset_folder, model)
        data = {}
        #print("WE ARE IN Shapes3dClassDataset CLASS in CORE PER BIN")
        for field_name, field in self.fields.items():
            try:
                if field_name == 'condition':
                    field_data = field.load(model_path, idx, self.binIdx)

                else:
                    field_data = field.load(model_path, idx)

                #print('field_data: ', field_data)
                if field_data == None: # Line Added by ERIK SANDSTROEM
                    #print('returns none!')
                    return None # Line Added by ERIK SANDSTROEM
            except Exception:
                if self.no_except:
                    logger.warn(
...
def collate_remove_none(batch):
    ...
    # my test shows that batch is never none, even though I return none for the batch. Aha the batch is an empty list in this case it appears. No, it seems to be an emtpy dictionary
    #print('batch before: ', batch) # when I can't find an image to condition on, this is [None]
    batch = list(filter(check_element_valid, batch))    
    #print('batch after: ', batch) # when I can't find an image to condtition on, this is []
    if isinstance(batch, list):# Line Added by ERIK SANDSTROEM
        if len(batch) == 0:# Line Added by ERIK SANDSTROEM
            return None# Line Added by ERIK SANDSTROEM
    return data.dataloader.default_collate(batch)


def check_element_valid(batch):
    #print('batch: ', batch) # This is None when no input image in the bin is found
    if batch is None:
        return False
