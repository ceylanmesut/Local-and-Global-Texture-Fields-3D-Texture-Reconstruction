'''
This script contains an evaluation function.
It simply takes real (groundtruth) images and generated images as input,
then in compares them using SSIM, Featl1 and FID metrics
'''
...
def evaluate_generated_images(metric, path_fake, path_real, batch_size=64):
    ...
    if metric == 'FID':
        ...
        value = SSIM.calculate_ssim_l1_given_paths(paths)
        #print("SSIM CALCULATION IS FINISHED")
        value_FID = FID.calculate_fid_given_paths(
            paths, batch_size, True, 2048)

        print("FID CALCULATION IS FINISHED")
