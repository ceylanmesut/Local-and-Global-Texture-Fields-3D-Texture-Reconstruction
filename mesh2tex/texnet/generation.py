...
class Generator3D(object):
    ...
    def __init__(self, model_g, device=None):
    ...
    def generate_images_4eval_condi_hd(self, batch, out_dir, model_names):
        ...
        #print("WE ARE IN GENERATE IMAGES_4EVAL_CONDI_HD")
        # Extract depth, gt, camera info, shape pc and condition
        ...
        #     print("keys of batch", key)

        # Berk: I added these new fields. (Berk wrote and now I added here in this function)
        ...
        for j in range(batch_size):
            for vidx in range(viewbatchnum):
                ...
                if len(condition_image.size()) == 1:
                    condition_ = condition_image[j:j+1].expand(
                                viewbatchsize)
                else:
                    condition_ = condition_image[j:j+1][:4].expand(
                        viewbatchsize, condition_image.size(1),
                        condition_image.size(2), condition_image.size(3)).to(self.device)

                # # Mesut added below if-else
                ...
                # Mesut added below if-else
                if len(condition_depth.size()) == 1:
                    condition_depth_ = condition_depth[j:j+1].expand(viewbatchsize)
                else:
                    condition_depth_ = condition_depth[j:j+1][:4].expand(viewbatchsize,
                                                    condition_depth.size(1),
                                                    condition_depth.size(2),
                                                    condition_depth.size(3))
                ...
                if len(condition_cam_K.size()) == 1:
                    condition_cam_K_ = condition_cam_K[j].expand(viewbatchsize) #previ num_views
                else:
                    condition_cam_K_ = condition_cam_K[j].expand(viewbatchsize,
                                                    condition_cam_K.size(1),
                                                    condition_cam_K.size(2))
                # Mesut added below if-else  
                if len(condition_cam_W.size()) == 1:
                    condition_cam_W_ = condition_cam_W[j].expand(viewbatchsize)
                else:
                    condition_cam_W_ = condition_cam_W[j].expand(viewbatchsize,
                ...
                # Generate images and save
                self.model_g.eval() # commented by Mesut
                with torch.no_grad():
                    # img_fake = self.model(depth_, cam_K_, cam_W_,
                    #                       geom_repr, condition_)
                    

                    # print("DEPTH SIZE", depth_.size())
                    # print("Cam K ", cam_K_.size())
                    # print("Cam W", cam_W_.size())
                    # print("Condition SIZE", condition_.size())
                    # print("Condition Depth SIZE", condition_depth_.size())
                    # print("Condition Cam K SIZE", condition_cam_K_.size())
                    # print("Condition Cam W SIZE", condition_cam_W_.size())
                    ...
                    # img_fake = self.model(depth_, cam_K_, cam_W_,
                    #                     geom_repr, condition_, condition_depth_,  condition_cam_K_ , condition_cam_W_) # returns dimension error the model itself
